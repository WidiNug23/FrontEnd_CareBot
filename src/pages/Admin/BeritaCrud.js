...
function BeritaCrud() {
  ...
  const handleCreateOrUpdate = async () => {
    ...
    try {
      ...
      if (newData.id) {
        ...
        response = await axios.put(`http://localhost:8080/berita/${newData.id}`, formData, {
        ...
        // Create new data
        response = await axios.post('http://localhost:8080/berita', formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/berita/${id}`);
