...
function IbuMenyusuiCrud() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/ibu_menyusui', newData);
  ...
  const handleEdit = (item) => {
    setEditId(item.id);
    setNewData({ nutrisi: item.nutrisi, sumber: item.sumber, jumlah: item.jumlah, deskripsi: item.deskripsi });
    setSuccess(null);
    setError(null);
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/ibu_menyusui/${editId}`, newData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
        const response = await axios.delete(`http://localhost:8080/ibu_menyusui/delete/${id}`);
